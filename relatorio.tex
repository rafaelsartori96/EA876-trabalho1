...
\geometry{a4paper,left=2.5cm,right=2.5cm,top=2cm,bottom=3cm} % menor margem
...
\section{Método}
    ...
    %A soma é simples, feita através da instrução \texttt{add r0, r0, r1}. Já a múltiplicação, temos o seguinte algoritmo: iniciando com zero em \texttt{r2}, somamos $1$ se $ \texttt{r0} > 0 $, $-1$ caso contrário, analogamente para \texttt{r1}; em \texttt{r3} e \texttt{r4}, respectivamente, os valores absolutos de \texttt{r0} e \texttt{r1}; trocamos \texttt{r3} e \texttt{r4} de lugar de modo a deixar o maior em \texttt{r3} e o menor em \texttt{r4}; com isso, copiamos \texttt{r3} para \texttt{r5} e fazemos enquanto $ \texttt{r4} > 1 $: sucessivas somas $ \texttt{r3} = \texttt{r3} + \texttt{r5} $ e subtraímos 1 de \texttt{r4}. Após conclusão desse laço, temos em \texttt{r3} o valor absoluto do resultado da multiplicação, usando o que calculamos em \texttt{r2}, identificamos o sinal final da resposta: se \texttt{r2} possui $-2$ ou $2$, a resposta deve possuir sinal positivo, caso possua $0$, a resposta deve possuir sinal negativo.
